...
class _InteractionalWidgetState extends State<InteractionalWidget> {
  ...
  Offset gyroscopeToOffset(double x, double y) {
    ...
    return Offset(
        (angleX / widget.maxAngleX) * maxBackgroundOffset.dx, (angleY / widget.maxAngleY) * maxBackgroundOffset.dy);
  }
      ...
      child: Stack(
        ...
        children: [
          Positioned(
              ...
              child: Transform.scale(scale: widget.backgroundScale ?? 1.0, child: widget.backgroundWidget ?? Row())),
          Positioned(child: Transform.scale(scale: widget.middleScale ?? 1.0, child: widget.middleWidget ?? Row())),
          Positioned(
              top: foregroundOffset.dy,
              left: foregroundOffset.dx,
              child: Transform.scale(scale: widget.foregroundScale ?? 1.0, child: widget.foregroundWidget ?? Row()))
        ],
      ),
    );
  }
